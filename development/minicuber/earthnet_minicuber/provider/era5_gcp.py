...
class ERA5_GCP(provider_base.Provider):
    ...
    def load_data(self, bbox, time_interval, **kwargs):
        ...
        band_long_names = {b: SHORT_TO_LONG_NAMES[b] for b in self.bands}
        era5 = era5[list(band_long_names.values())]
        ...
        lat_name = "latitude" if "latitude" in era5.coords else "lat"
        lon_name = "longitude" if "longitude" in era5.coords else "lon"

        era5 = era5.sel({lat_name: center_lat, lon_name: center_lon}, method="nearest").drop_vars([lat_name, lon_name])
        ...
        for aggregation_type in self.aggregation_types:
            ...
            rename_dict = {v: f"era5land_{k}_{aggregation_type}" for k, v in band_long_names.items()}
            curr_agg_era5 = curr_agg_era5.rename(rename_dict)
